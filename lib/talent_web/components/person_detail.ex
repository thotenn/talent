defmodule TalentWeb.Components.PersonDetail do
  ...
  def person_detail(assigns) do
    # Verificar si la persona está correctamente cargada
    assigns = if is_nil(assigns.person) || is_struct(assigns.person, Ecto.Association.NotLoaded) do
      Map.put(assigns, :person_loaded, false)
    else
      Map.put(assigns, :person_loaded, true)
    end
      ...
      <% else %>
        <div class="border-t border-gray-200">
          <dl>
            ...
            <!-- Redes sociales -->
            <% has_networks = Ecto.assoc_loaded?(@person.person_networks) && Enum.any?(@person.person_networks) %>
            <%= if has_networks do %>
              <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                <dt class="text-sm font-medium text-gray-500">Redes sociales</dt>
                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                  <ul class="border border-gray-200 rounded-md divide-y divide-gray-200">
                    <%= for network <- @person.person_networks do %>
                      <li class="pl-3 pr-4 py-3 flex items-center justify-between text-sm">
                        <div class="w-0 flex-1 flex items-center">
                          <span class="flex-1 w-0 truncate">
                            <%= if Ecto.assoc_loaded?(network.network) do %>
                              <span class="font-medium"><%= network.network.name %>:</span>
                            <% else %>
                              <span class="font-medium">Red social:</span>
                            <% end %>
                            <%= network.username %>
                          </span>
                        </div>
